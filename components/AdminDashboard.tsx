...
const AdminDashboard: React.FC = () => {
  const [userAccounts, setUserAccounts] = useState<FullUserData[]>([]);
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      try {
        const allData = await firebaseService.listAllUsers();
        setUserAccounts(allData || []);
      } catch (e) {
  ...
  const globalStats = {
    ...
    activeAccounts: userAccounts.filter(u => u.userProfile?.account_status === 'active' || u.userProfile?.account_status === 'admin').length,
    totalVolume: userAccounts.reduce((acc, curr) => {
      const userSpent = (curr.history || []).reduce((sum, h) => sum + (h.meta?.total_spent || 0), 0);
      return acc + userSpent;
    }, 0),
    codesInUse: Array.from(new Set(userAccounts.map(u => u.userProfile?.promo_code).filter(Boolean))).length
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard icon="fa-users" label="Utilizadores" value={globalStats.totalUsers} color="indigo" />
        <StatCard icon="fa-bolt" label="Contas Ativas" value={globalStats.activeAccounts} color="emerald" />
        <StatCard icon="fa-vault" label="Volume Global" value={`€${globalStats.totalVolume.toFixed(2)}`} color="purple" />
        <StatCard icon="fa-ticket" label="Códigos em Uso" value={globalStats.codesInUse} color="amber" />
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-50">
              {loading ? (
                <tr><td colSpan={5} className="p-20 text-center text-slate-300 font-black animate-pulse">A carregar base de dados...</td></tr>
              ) : userAccounts.length === 0 ? (
                <tr><td colSpan={5} className="p-20 text-center text-slate-300 font-black italic">Sem utilizadores na cloud real.</td></tr>
              ) : userAccounts.map((account, i) => {
                const u = account.userProfile;
                const userVolume = (account.history || []).reduce((sum, h) => sum + (h.meta?.total_spent || 0), 0);
                return (
                  <tr key={i} className="hover:bg-indigo-50/30 transition-colors">
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-3">
                        ...
                        {u?.role === 'owner' && (
                          <i className="fa-solid fa-crown text-amber-500 text-[10px]" title="Plataform Owner"></i>
                        )}
                      </div>
                    </td>
                    <td className="px-8 py-6">
