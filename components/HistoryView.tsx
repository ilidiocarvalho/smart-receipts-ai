...
const HistoryView: React.FC<HistoryViewProps> = ({ history, isCloudActive, onSelectReceipt }) => {
  return (
      ...
      {history.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {history.map((item) => (
            <div 
              key={item.id} 
              className="bg-white p-7 rounded-[2rem] border border-slate-200 flex items-center justify-between hover:border-indigo-500 hover:shadow-2xl hover:shadow-indigo-100/50 transition-all cursor-pointer group" 
